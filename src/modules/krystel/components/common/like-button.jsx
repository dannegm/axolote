...
export default function LikeButton({ quote, settings }) {
    return (
        <LikeHandler quote={quote} settings={settings}>
            <button
                type='button'
                className='flex flex-row gap-2 justify-center items-center px-6 py-3 rounded-full w-14 h-14 shadow-md bg-red-500 text-white hover:shadow-lg hover:bg-red-600 active:shadow-2xs transition-all duration-200 hover:scale-125 active:scale-100'
            >
                <div className='block'>
                    <Heart size={20} />
                </div>
