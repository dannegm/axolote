...
const filteredData = [...new Set(data)] // Elimina duplicados
    .filter(item => item.trim())
    .filter(item => /^[a-z]/.test(item))
    .map(item => item.trim()); // Filtra los que inician con minúscula

const output = `export const tailwind = ${JSON.stringify(filteredData, null, 2)};`;

fs.writeFileSync('cleanArray.js', output);

console.log('Archivo "cleanArray.js" generado correctamente.');
